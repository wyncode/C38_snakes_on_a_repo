...
const PetEmail = (subject, message, toEmail, name, pet, fromID) => {
  sgMail
    .send({
      to: toEmail,
      from: `${process.env.FROM_EMAIL}`,
      subject: `From Petster Exotic: ${pet.name}'s Information`,
      html: `
      <div style="font-family: sans-serif">
      ...
      <p>
          ...
          <a target="_blank" rel="noopener noreferrer" href="${process.env.URL}/petprofile/${pet._id}">
              ${pet.name}</a>'s
          information from
          <a href="${process.env.URL}" target="_blank" rel="noopener noreferrer">Petster Exotic</a>!
      </p> 
      <h4>${subject}</h4>
      ...
      rel="noopener noreferrer">Petster Exotic</a>!
      </h3>
  </div>`
    })
    .catch((error) => console.log(error));
